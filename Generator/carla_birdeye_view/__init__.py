...
# 336px -> 84m
DEFAULT_HEIGHT = 200  # its 50m when density is 4px/m
DEFAULT_WIDTH = 200  # its 37.5m when density is 4px/m
...
class BirdViewProducer:
    ...
    def produce(self, agent_vehicle: carla.Actor) -> BirdView:
        ...
        masks, ids, corners = self._render_actors_masks(agent_vehicle, segregated_actors, masks)
        cropped_masks, w, h = self.apply_agent_following_transformation_to_masks(
            agent_vehicle, masks
        )

        coords, indexes = self.extractor(agent_vehicle, corners, ids, w, h)
        ordered_indices = [mask.value for mask in BirdViewMasks.bottom_to_top()]
        return cropped_masks[:, :, ordered_indices], indexes, coords
    ...
    def _render_actors_masks(
        ...
        masks[BirdViewMasks.VEHICLES.value], ids, corners = self.masks_generator.vehicles_mask(
        ...
        return masks, ids, corners

    def extractor(self, agent_vehicle: carla.Actor, coords, ids, w, h):
        ...
        for coord in coords:
            ...
            rotated_coord.append(M.dot(points_ones.T).T)

        half_width = self.target_size.width // 2
        hslice = (rotation_center.x - half_width, rotation_center.x + half_width)
        ...
        final_coords, indexes = [], []
        for index, coord in enumerate(rotated_coord):
            flag = True
            new_corner = []
            for corner in coord:
                x, y = corner[0], corner[1]
                if not((x >= hslice[0] and x <= hslice[1]) and (y >= vslice[0] and y <= vslice[1])):
                    flag = False
                new_corner.append([x - hslice[0], y - vslice[0]])
            if flag:
                final_coords.append(new_corner)
                indexes.append(ids[index])
        return final_coords, indexes
