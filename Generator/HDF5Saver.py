...
class HDF5Saver:
    def __init__(self, sensor_width, sensor_height, file_path_to_save="data/carla_dataset.hdf5"):
        ...
        self.birdview_group = self.file.create_group("birdview")
        self.vehicle_ids_rgb = self.file.create_group("vehicle_ids_rgb")
        self.vehicle_ids_birdview = self.file.create_group("vehicle_ids_birdview")
        self.bb_bv = self.file.create_group("bb_bv") # bounding box coordinates in birdview image
        self.names = self.file.create_group("names")
        self.distances = self.file.create_group("distances")
    ...
    def record_data(self,
        ...
        self.birdview_group.create_dataset(timestamp, data=birdview_frame)
        self.vehicle_ids_rgb.create_dataset(timestamp, data=vehicle_ids_rgb)
        self.vehicle_ids_birdview.create_dataset(timestamp, data=vehicle_ids_birdview)
        self.bb_bv.create_dataset(timestamp, data=bb_bv)
        self.names.create_dataset(timestamp, data=names)
        self.distances.create_dataset(timestamp, data=distances)
